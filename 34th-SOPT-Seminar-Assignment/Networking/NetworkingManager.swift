...
final class NetworkingManager {
    ...
    /*
     ...
     만약 NetworkingManager를 구조체로 만들 경우, 여러 개의 인스턴스를 생성해도 함수의 기능을 구현하는 데에는 문제가 없지만, 새로운 인스턴스가 생성되면 캐시데이터를 가져올 수 없음.
     (캐시 데이터를 CoreData에 저장하면 가능은 할 듯.) 그래서 메모리에서 계속 캐시데이터를 갖고 있을 수 있도록 싱글톤 객체를 생성한 것임..
     
     -> 그러면 저장 속성 말고 타입 저장 속성(static let, static var)으로 구현하면 되지 않느냐..!! 라는 질문에는...글쎼...
     ...
     반대로 앱이 시작하면서 싱글톤 객체가 생성될 때 코어데이터에서 캐시데이터를 가져오는 방식으로 구현 (한 번에 많은 양의 캐시데이터를 불러오는 게 부담스러울 것 같으면 접근 시마다 코어데이터에 있는지 확인하고 캐시데이터로 복사해 오던가 등등...의 방법으로도 구현할 수 있을 것 같다.)
     */
    
    let moyaProvider = MoyaProvider<KOBISAPI>()
    var cacheData: [String: [DailyBoxOfficeMovie]] = [:] // 키는 "yyyyMMdd" 포맷의 문자열, 값은 DailyBoxOfficeMovie타입을 요소로 갖는 딕셔너리
    ...
    func getAPI(dayBeforeToday: UInt, contentsNum: Int = 10, completion: @escaping ([DailyBoxOfficeMovie]) -> Void) {
        
        let currentDate = Date.now
        guard let targetDate = Calendar(identifier: .gregorian).date(byAdding: .day, value: -Int(dayBeforeToday), to: currentDate) else { fatalError() }
        ...
        /*
         ...
         캐시데이터에 있으면 네트워크 통신 없이 캐시데이터를 반환
         */
        if self.cacheData.keys.contains(dateInString) {
            guard let dailyBoxOfficeList = cacheData[dateInString] else { fatalError() }
        ...
        self.moyaProvider.request(KOBISAPI.getMovieRanking(targetDate: dateInString, number: contentsNum)) { result in
            ...
            case .success(let result):
                ...
                do {
                    ...
                    if !self.cacheData.keys.contains(dateInString) {
                        self.cacheData[dateInString] = decodedNetworkingResult.boxOfficeResult.dailyBoxOfficeList
                    }
